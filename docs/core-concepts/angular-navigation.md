...
Forward navigation can be also called downward navigation since you are going down in your navigation hierarchy. This is the default form of navigation in an Angular application and is done using the **Angular Component Router**. You can check [this detailed guide on how to use the router](https://angular.io/guide/router). From here on we are going to assume that you are familiar with the basic concepts and concentrate on the specifics when doing forward navigation with Angular inside a NativeScript app.
